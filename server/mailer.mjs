...
export async function sendMail({ to, subject, text, html }) {
  ...
  await getTransport().sendMail({
    from: config.mailFrom || `${BRAND_NAME} <${config.smtpUser}>`,
    to,
    subject,
    text,
    html,
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="padding:24px 0">
    <tr><td align="center">
      <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden">
        <tr><td style="background:#15803d;padding:18px 24px;color:#ffffff;font-size:16px;font-weight:bold">${esc(BRAND_NAME)}</td></tr>
...
export function signupCodeMail({ code, minutes }) {
  return {
...
export function recoveryMail({ link }) {
  return {
